...
export function formatPrice(price: number | string | null | undefined, locale: string = "en"): string {
  ...
  // Always use English numbers (1,234.56) for both EN and AR
...
export function parseMagentoQueryParams(
  ...
  let page = 1;
  let sortBy = "none";
  const reserved = ["categoryId", "page", "pageSize", "sortBy", "is_ajax", "storeId", "store", "storeCode", "category", "searchBy", "search", "item_code", "itemCode"];
