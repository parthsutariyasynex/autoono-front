...
//         if (res?.ok) {
//           // Set auth-token cookie as a robust fallback for middleware
//           if (magentoToken) {
//             const cleanToken = String(magentoToken).replace(/"/g, "").trim();
//             document.cookie = `auth-token=${cleanToken}; Path=/; Max-Age=${60 * 60 * 24 * 30}; SameSite=Lax${window.location.protocol === 'https:' ? '; Secure' : ''}`;
//           }
...
//           // Set auth-token cookie as a robust fallback for middleware
//           if (otpToken) {
//             const cleanToken = String(otpToken).replace(/"/g, "").trim();
//             document.cookie = `auth-token=${cleanToken}; Path=/; Max-Age=${60 * 60 * 24 * 30}; SameSite=Lax${window.location.protocol === 'https:' ? '; Secure' : ''}`;
//           }
...
//           <div className="px-4 sm:px-6 md:px-8 pt-7 pb-5">
...
//                     <label className="block text-[13px] font-bold text-gray-900 uppercase tracking-tight cursor-pointer">
